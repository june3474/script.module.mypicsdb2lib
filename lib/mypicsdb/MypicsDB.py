...
class MyPictureDB(object):
    ...
    def make_new_base(self, ecrase=True):
        ...
        common.log("make_new_base >> Dropping table", "starting", xbmc.LOGINFO)
        if ecrase:
            #drop table
        ...
        # table: version
        try:
            common.log("MPDB.make_new_base >> Dropping table", "CREATE TABLE DBVersion ( strVersion VARCHAR(20))", xbmc.LOGINFO)
        ...
        for tag_type, value in dictionary.items():
            ...
            if tag_type in dictionary:
                ...
                if tag_type not in ['sha', 'strFilename', #'strPath',
                                   'mtime', #'ftype',
                                   'source', 'urgency', 'time created', 'date created', 'idFolder']:
    
                    tag_values = value.split(lists_separator)
    ...
    def filterwizard_result(self, set_tags, unset_tags, match_all, start_date='', end_date='', min_rating = 0):
        ...
        common.log("", "match_all = %s"%(match_all))
        ...
        if match_all == 1 or match_all == "1":
            if len(set_tags) > 0:
                for filter_tags in set_tags_array:
                    ...
                    condition = "AND tt.TagTranslation = '"+urllib.parse.unquote_plus(key)+"' AND tc.TagContent = '"+urllib.parse.unquote_plus(value)+"' "
            ...
            if len(set_tags) > 0:
                ...
                for filter_tags in set_tags_array:
                    ...
                    if key != old_key:
                        if len(old_key) > 0:
                            condition = "AND tt.TagTranslation = '"+urllib.parse.unquote_plus(old_key)+"' AND tc.TagContent in( "+urllib.parse.unquote_plus(old_value)+" ) "
                            outer_select += " AND idFile in ( " + inner_select + condition + " ) "
                        old_key = key
                        old_value = "'" + value + "'"
                    else:
        ...
        common.log('filterwizard_result', outer_select, xbmc.LOGINFO)
    ...
    def filterwizard_save_filter(self, filter_name, items, match_all, start_date ='', end_date = ''):
        ...
        common.log("MyPictureDB.filterwizard_save_filter", filter_name)
        if self.db_backend.lower() == 'mysql':
        ...
        common.log("MyPictureDB.filterwizard_save_filter", "End")
        self.con.commit()
    ...
    def delete_root(self, path):
        ...
        common.log( "delete_root", "name = %s"%path)
    ...
    def set_tagtype_translation(self, TagType, TagTranslation):
        self.cur.request("Update TagTypes set TagTranslation = ? where TagType = ? ",(urllib.parse.unquote_plus(TagTranslation), urllib.parse.unquote_plus(TagType)))
        self.con.commit()
    ...
    def list_tags(self,tagType):
        ...
        return [row for (row,) in self.cur.request( """select distinct TagContent from TagContents tc, TagsInFiles tif, TagTypes tt  where tc.idTagContent = tif.idTagContent and tc.idTagType = tt.idTagType and tt.TagTranslation='%s' ORDER BY LOWER(TagContent) ASC"""%tagType )]
    ...
    def list_tags_count(self, tagType, min_rating=0):
        ...
        group BY TagContent""",(tagType,) )]
         ...
         where tc.idTagContent = tif.idTagContent
           ...
           and tc.idTagType = tt.idTagType 
           and tt.TagTranslation = ? 
        group BY TagContent""",(min_rating, tagType,) )]
    ...
    def all_children_of_folder(self, rootid):
        ...
        Returns (list): idFolder values of all subfolders under rootid.
                        If there is no subfolder, returns an empty list. 
        """        
        ...
        try:
            ...
            for row in rows:
                ...
                if(len(str1) == 0 ):
                    ...
                    str1 = str1 + ' / ' + dummy
            common.log("get_pic_persons",  str1)
            return (str1)
            #return str(rows)
        except Exception as msg:
            common.log("",  "%s - %s"%(Exception,msg), xbmc.LOGERROR )
