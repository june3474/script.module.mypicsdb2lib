...
def run_plugin(plugin, params=""):
    if params != "":
        ...
        xbmc.executebuiltin('RunPlugin(%s?%s)'%(smart_utf8(plugin), smart_utf8(quoted_params)))
    else:
        xbmc.executebuiltin('RunPlugin(%s)'%smart_utf8(plugin))


def run_script(script):
    log("common.run_script", 'RunScript(%s)'%script)
...
def quote_param(parm):
    ...
    parm = urllib.parse.quote_plus(parm)

    return parm


def unquote_param(parm):
    parm = urllib.parse.unquote_plus(parm)
